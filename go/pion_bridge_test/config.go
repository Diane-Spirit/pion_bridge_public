...
// WebSocketConfig holds configuration specific to the WebSocket server.
type WebSocketConfig struct {
	Address string `yaml:"address"` // Address for the WebSocket server to listen on (e.g., ":8080")
}
...
func GetConfig() (*AppConfig, error) {
	loadOnce.Do(func() {
		...
		if cfg.WebSocket.Address == "" {
			loadConfigErr = errors.New("Missing configuration: WebSocket.address")
			return
		}
